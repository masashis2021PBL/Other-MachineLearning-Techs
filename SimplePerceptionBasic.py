...
def step (x):
   ...
   return relu (x, alpha = 0.0, max_value = 1, threshold = 0.0)
...
### 出力層の重み、バイアスの設定 - ORゲート
output_layer = model.layers [0]
output_layer_weights = output_layer.get_weights ()
output_layer_weights [0][0][0] = 0.5    # x1に対する重み： 0.5
output_layer_weights [0][1][0] = 0.5    # x2に対する重み： 0.5
output_layer_weights [1][0] = -0.5      # バイアス： -0.5
